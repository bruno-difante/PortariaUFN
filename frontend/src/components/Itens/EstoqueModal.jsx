...
const EstoqueModal = ({ isOpen, onClose }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            fetch("http://192.168.100.97:8080/itens")
                ...
                .catch(err => {
                    console.error(err);
                    setMensagemAlerta("Erro ao carregar itens!");
                });
        }
    }, [isOpen]);
    ...
    const deletarSelecionados = async () => {
        try {
            ...
            setMensagemAlerta("Itens deletados com sucesso!");
        } catch (err) {
            console.error(err);
            setMensagemAlerta("Erro ao deletar os itens.");
        }
    };
    ...
    const atualizarItem = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setEditandoId(null);
                setFormEdicao({});
                setMensagemAlerta("Item atualizado com sucesso!");
            } else {
                setMensagemAlerta("Erro ao atualizar item!");
            }
        } catch (error) {
            console.error(error);
            setMensagemAlerta("Erro na requisição.");
        }
    };
